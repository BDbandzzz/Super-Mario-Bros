...
class Juego:
    ...
    def movimiento_scroll(self):
        subida = 0
        bajada = 0
        self.salto_movimiento = False
        
        if self.personaje.esta_saltando and (self.personaje.running or self.personaje.walking):
            self.salto_movimiento = True
        
        if (self.salto_movimiento or 
            (self.personaje.walking or self.personaje.running)
            and not self.personaje.esta_agachado):
    ...
    def bucle_principal(self):
        while self.juego_activo:
            ...
            if not self.juego_pausado:
                ...
                self.drop_hongos() 
                self.drop_vidas() 
                self.drop_enemigos() if self.contador_enemigos < 10 else None
